...
from automation.changeNicknameTest import run_change_nickname
...
router = APIRouter(
    prefix="/api/change-nickname",
    tags=["change-nickname"],
)
...
# Single device nickname change
@router.post("/single")
async def change_nickname_single(
    request: NicknameRequest,
    background_tasks: BackgroundTasks = BackgroundTasks()
):
    """
    Change nickname for a single device
    """
    try:
        # Process device data
        ...
        task_id = f"nickname_{device_id}_{uuid.uuid4().hex[:8]}"
        ...
        executor.submit(run_nickname_change_task, device_data, request.nickname, callback, task_id, current_loop)
        
        return {"success": True, "task_id": task_id, "message": f"Nickname change task started for device {device_id}"}
    except Exception as e:
        logger.error(f"Error in change_nickname_single: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error starting nickname change: {str(e)}")

# Stop ongoing tasks
@router.post("/stop")
async def stop_nickname_change(task_id: Optional[str] = None):
    """Stop all ongoing nickname change tasks or a specific task"""
    try:
        ...
        if not active_workers:
            return {"status": "info", "message": "No active nickname change tasks to stop"}
        
        for worker in active_workers:
            if hasattr(worker, 'set_stopping') and callable(worker.set_stopping):
                worker.set_stopping(True)
        
        await manager.broadcast_to_status("Stopping all nickname change tasks")
        
        return {"status": "success", "message": "Stopping all nickname change tasks"}
    except Exception as e:
        logger.error(f"Error in stop_nickname_change: {e}")
        raise HTTPException(status_code=500, detail=str(e))

# Helper functions
def run_nickname_change_task(device_data, nickname, callback, task_id, main_loop):
    """Run the nickname change task for a single device"""
    try:
        device_id = f"{device_data['device_ip']}:{device_data['u2_port']}"
        logger.info(f"Starting nickname change for device {device_id}")
        ...
        active_tasks[task_id] = {
            ...
            "status": "running",
            "callback": callback,
            "messages": [f"Starting nickname change for device {device_id}"]
        }
        ...
        # Call the nickname change function
        result = run_change_nickname(
            callback,
        ...
        try:
            asyncio.run_coroutine_threadsafe(
                manager.broadcast_to_task(task_id, f"Nickname change for device {device_id} completed with result: {result}"),
                main_loop
            )
            ...
            asyncio.run_coroutine_threadsafe(
                manager.emit_to_task(task_id, "completed", {
                    "message": f"Nickname change for device {device_id} {'succeeded' if result else 'failed'}"
                }),
                main_loop
            )
        ...
        logger.error(f"Error in run_nickname_change_task: {e}", exc_info=True)
        # Send error message via WebSocket
        try:
            # Use the main loop passed from the calling function
            asyncio.run_coroutine_threadsafe(
                manager.broadcast_to_task(task_id, f"Error changing nickname for device {device_id}: {str(e)}"),
                main_loop
            )
            # Update task status
            if task_id in active_tasks:
